 ...
 * that allows the user to select a task data directory to use.
 * 
...
public class SwitchTaskDataFolderAction extends Action implements IViewActionDelegate {
	...
	/**
	 * Prompts the user for the name of the task data folder to switch
	 * to. Returns the string selected by the user or the empty string
	...
	/**
	 * Returns an array of the string names of the folders in the main
	 * data directory plus the default folder.
	 * 
	 * This performs validation and will alert the user of the following
	 * failures:
	 * - No shared task folder specified
	...
	public String[] getFolderStrings(){
		try {
			...
			if (sharedDataPath.trim().equals("")){
				MessageDialog.openError(shell, "Switch Task Folder Error", "Please specify a root shared task folder in the Mylar Reports preference page.");
				return null;
			}
			...
			if (!rootDir.exists() || !rootDir.isDirectory()){
				MessageDialog.openError(shell, "Switch Task Folder Error", "The root folder specified in the Mylar Reports preference page could not be found.");
				return null;
			}
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (currFile.isDirectory() && containsTaskData(currFile) && !MylarPlugin.getDefault().getMylarDataDirectory().endsWith(currFile.getName())){
			...
			if (!(folders.size() > 0)){
				MessageDialog.openError(shell, "Switch Task Folder Error", "No task data folders were found in the root folder specified in the Mylar Reports preference page.");
				return null;
			}
	...
	/**
	 * Returns true if the specified folder contains task data.
	 * Currently just checks if it contains a task list file.
	 */
	...
	/**
	 * Switches the current data folder to the specified folder.
	 * Specify only the folder name and not the full path.
	 * 
	 * Note: This currently always resumes capture when returning
	 * to the main data directory (and suspends capture when
	...
	public void switchTaskDataFolder(String targetFolder){
		...
		if (targetFolder.equals(MAIN_LOCAL_DATA_DIR)){
			...
			String dataDirPath = MylarReportsPlugin.getDefault().getRootSharedDataDirectory() + File.separator + targetFolder;
