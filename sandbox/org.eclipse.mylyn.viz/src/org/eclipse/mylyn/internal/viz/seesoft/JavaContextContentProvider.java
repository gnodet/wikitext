...
/**
 * Provides interesting java packages and classes for display in the Visualiser
 * view.
 * 
...
public class JavaContextContentProvider implements IContentProvider, IMylarContextListener {
	...
	/**
	 * Constructs the model to be displayed in the visualiser by requesting the
	 * active context's interesting elements and converting them to the groups
	 * and members structure
	 */
	...
	/**
	 * Adds the specified visualiser member to the group representing its
	 * package in the groups list. The package group is created as required.
	 */
	protected void addTypeToGroups(JDTMember javaType) {
		...
		if (packageElt.getElementType() != IJavaElement.PACKAGE_FRAGMENT) {
			MylarStatusHandler.fail(new Exception("Could not find package for java element"),
					"Could not find package for java element", false);
			return;
		}
	 ...
	 *      boolean, int) Copied from JDTContentProvider
	 */
